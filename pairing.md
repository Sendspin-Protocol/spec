...
**Digits emission.** A client that displays the pairing code follows [Pairing Code Presentation](#pairing-code-presentation). A client that speaks it SHOULD use the best-matching language it supports, treating the server's [`languages`](messaging.md#server--client-serverhello) as the language priority list under [RFC 4647](https://www.rfc-editor.org/rfc/rfc4647#section-3.4) Lookup matching, and falling back to its own default when nothing matches. The hint is informational and never grounds for [`pair/abort`](#client--server-pairabort); display emission is unaffected. Spoken emission reads single digits in the [presentation groups](#pairing-code-presentation) and SHOULD leave a short gap between digits and a longer one between groups.
